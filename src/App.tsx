...
export function App() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // ВАЖНО: в режиме просмотра НЕ берём почту из вашей сессии —
        // права считаются только от просматриваемого профиля
        if (!meEmail && !inViewAs) {
  ...
  // в режиме просмотра переключение ролей отключено
  const role: 'tenant' | 'landlord' = inViewAs ? baseRole : (roleOverride || baseRole)
  const view: 'tenant' | 'landlord' | 'admin' = inViewAs ? baseRole : (adminView && isAdmin ? 'admin' : role)
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '10px 10px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'stretch', background: 'rgba(120,120,128,0.12)', borderRadius: 14, padding: 6, margin: '0 0 10px', overflowX: 'auto' }}>
          {!adminReady ? (
            ...
            <>
              {/* режим просмотра: только роль просматриваемого пользователя, без переключателей и админки */}
