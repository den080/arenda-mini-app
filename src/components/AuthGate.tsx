...
const ADMIN_EMAILS = ['roomiobot@mail.ru']

// Экран первого входа с сайта: создаёт профиль, если его ещё нет
function OnboardCard({ onCreated }: { onCreated: () => void }) {
  const [busy, setBusy] = useState<string | null>(null)
  async function create(role: 'tenant' | 'landlord') {
    setBusy(role)
    try {
      const { data: s } = await supabase.auth.getSession()
      const su = s?.session?.user
      if (!su) { showToast('Сессия не найдена — обновите страницу'); return }
      const email = String(su.email || '').toLowerCase()
      const meta = (su.user_metadata || {}) as any
  ...
  const btn = (active: boolean) => ({
    ...
    background: active ? '#0071e3' : '#e8e8ed', color: active ? '#fff' : '#1d1d1f',
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 14, padding: 20, marginTop: 10, textAlign: 'center' }}>
      <div style={{ fontSize: 16, fontWeight: 600, color: '#1d1d1f', marginBottom: 8 }}>Добро пожаловать в Roomio!</div>
      <div style={{ fontSize: 14, color: '#8e8e93', marginBottom: 14 }}>
        Профиль ещё не создан. Выберите роль — и вы попадёте в приложение.
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <button disabled={!!busy} onClick={() => create('landlord')} style={btn(true)}>Я арендодатель</button>
        <button disabled={!!busy} onClick={() => create('tenant')} style={btn(false)}>Я арендатор</button>
      </div>
      <button onClick={onCreated} style={{ border: 'none', background: 'transparent', color: '#0071e3', fontSize: 13, fontWeight: 600, cursor: 'pointer', marginTop: 12, padding: 4 }}>
        Профиль уже есть — повторить поиск
      </button>
    </div>
  )
}
...
export function App() {
  const { user, refresh, loading } = useTelegramUser() as any
  ...
  useEffect(() => { (setErrorUser as any)(user) }, [user])

  useEffect(() => {
    if (!user) return
    const em = String(user.email || '').toLowerCase()
    const adm = ADMIN_EMAILS.includes(em)
    setIsAdmin(adm)
    setAdminReady(true)
    try {
      localStorage.setItem('roomio_admin_v2', adm ? '1' : '0')
  ...
  const view = adminView && isAdmin ? 'admin' : role
  ...
  const segFlex = (a: boolean): CSSProperties => ({
  ...
  const iconBtn: CSSProperties = {
    border: 'none', background: 'transparent', fontSize: 18, cursor: 'pointer', padding: '0 10px', borderRadius: 10,
  }
  const fixedBtn = (a: boolean): CSSProperties => ({
    border: 'none', background: a ? '#0071e3' : 'rgba(0,113,227,0.12)', color: a ? '#fff' : '#0071e3',
    fontWeight: 700, fontSize: 15, borderRadius: 12, padding: '10px 14px', cursor: 'pointer', whiteSpace: 'nowrap',
  })
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '10px 10px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'stretch', background: 'rgba(120,120,128,0.12)', borderRadius: 14, padding: 6, margin: '0 0 10px', overflowX: 'auto' }}>
          {!adminReady ? (
            ...
            <>
              <button style={segFlex(true)} disabled>{baseRole === 'landlord' ? 'Арендодатель' : 'Арендатор'}</button>
        ...
        {!user ? (
          loading ? (
            <div style={{ background: '#fff', borderRadius: 14, padding: 20, marginTop: 10, textAlign: 'center', color: '#8e8e93', fontSize: 14 }}>Загрузка…</div>
          ) : (
            <OnboardCard onCreated={refresh} />
          )
        ) : view === 'tenant' ? (
          <TenantDashboard />
        ) : view === 'landlord' ? (
      ...
      <Modal open={fbOpen} title="Обратная связь" onClose={() => setFbOpen(false)}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            style={{ flex: 1, padding: 12, borderRadius: 10, border: 'none', background: '#0071e3', color: '#fff', fontWeight: 700, fontSize: 15, opacity: fbBusy ? 0.6 : 1, cursor: 'pointer' }}
