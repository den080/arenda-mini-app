...
import { T } from '../theme'
import { Modal, showToast, Toaster, Hint, errText } from './ui'
...
const blueBtn: React.CSSProperties = { width: '100%', padding: 13, borderRadius: 12, border: 'none', background: '#0071e3', color: '#fff', fontWeight: 700, fontSize: 16, cursor: 'pointer', boxSizing: 'border-box', margin: '10px 0 0' }
const grayBtn: React.CSSProperties = { width: '100%', padding: 13, borderRadius: 12, border: 'none', background: '#e8e8ed', fontWeight: 600, fontSize: 16, cursor: 'pointer', margin: '8px 0 0' }
const demoBtn: React.CSSProperties = { border: 'none', background: 'transparent', color: '#0071e3', fontSize: 14, fontWeight: 600, cursor: 'pointer', padding: 4 }
...
const h0: React.CSSProperties = { fontSize: 20, fontWeight: 700, color: '#1d1d1f', marginBottom: 6, lineHeight: 1.25 }
...
const POLICY = [
  ...
  'Хранение: защищённая облачная база; доступ — только вы, ваш контрагент по договору и владелец сервиса для поддержки. Передача третьим лицам не осуществляется, кроме случаев, требуемых законом.',
  'Срок: до удаления аккаунта или договора.',
  'Ваши права: запросить, изменить, удалить данные — через владельца сервиса.',
].join('\n\n')

// Тестовое окно ЮKassa — ТОЛЬКО для покупки подписки Pro в демо
function MockYooKassa({ title, amount, onPay }: { title: string; amount: string; onPay: () => void }) {
  return (
    <div style={{ border: '1px solid #e3e3e8', borderRadius: 12, padding: 12, margin: '12px 0 4px' }}>
      <div style={{ fontSize: 12, color: '#8e8e93', marginBottom: 6 }}>ЮKassa · окно оплаты (демо)</div>
      ...
      <Hint text="В реальном приложении подписка Pro оплачивается через ЮKassa так же. Платежи по аренде идут напрямую между сторонами." />
    </div>
  )
}
  ...
  return (
    <div style={{ ...T.page, maxWidth: 480, margin: '0 auto', paddingBottom: 40 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, background: 'rgba(255,149,0,0.15)', borderRadius: 12, padding: '10px 12px', margin: '8px 0 12px' }}>
        <span style={{ fontSize: 14, fontWeight: 600, color: '#b25000' }}>Демо: продукт и сценарии оплат без регистрации</span>
      ...
      {role === 'tenant' ? (
        <>
          <div style={T.card}>
            ...
            <div style={dRow}><span style={{ fontSize: 16, fontWeight: 600 }}>Реутов, ул. Лесная д.11, кв.4</span></div>
          ...
          <div style={T.card}>
            ...
            <div style={dRow}><span style={{ fontSize: 16 }}>Аренда</span><span style={{ fontSize: 16, fontWeight: 600 }}>67 000 ₽</span></div>
            <div style={dRow}><span style={{ fontSize: 16 }}>Ресурсы по квитанции</span><span style={{ fontSize: 16, fontWeight: 600 }}>4 200 ₽</span></div>
            <div style={{ ...dRow, borderBottom: 'none' }}><span style={{ fontSize: 16, fontWeight: 700 }}>Итого</span><span style={{ fontSize: 16, fontWeight: 700 }}>71 200 ₽</span></div>
            {!rentPaid && (
              <>
                <div style={{ border: '1px solid #e3e3e8', borderRadius: 12, padding: 12, margin: '12px 0 4px' }}>
                  <div style={{ fontSize: 12, color: '#8e8e93', marginBottom: 6 }}>Реквизиты арендодателя</div>
          ...
          <div style={T.card}>
            ...
            <div style={dRow}><span style={{ fontSize: 16 }}>Холодная вода · ХВ-034944</span><span style={{ fontSize: 14, color: '#8e8e93' }}>прошлые: 322</span></div>
            <div style={{ ...dRow, borderBottom: 'none' }}><span style={{ fontSize: 16 }}>Горячая вода · ГВ-724271</span><span style={{ fontSize: 14, color: '#8e8e93' }}>прошлые: 24</span></div>
        ...
        <>
          ...
          <div style={T.card}>
            ...
            <div style={dRow}><span style={{ fontSize: 16, fontWeight: 600 }}>Реутов, ул. Лесная д.11, кв.4</span><span style={{ fontSize: 13, color: '#080' }}>До оплаты 8 дн. · 67 000 ₽</span></div>
            <div style={{ ...dRow, borderBottom: 'none' }}><span style={{ fontSize: 16, fontWeight: 600 }}>Москва, ул. Тверская 7, кв.12</span><span style={{ fontSize: 13, color: '#a80' }}>Ждём показания · 45 000 ₽</span></div>
          </div>
          <div style={T.card}>
            <div style={T.h2}>Замороженные штрафы</div>
            <div style={{ ...dRow, borderBottom: 'none' }}><span style={{ fontSize: 16 }}>август · просрочка показаний</span><span style={{ fontSize: 16, fontWeight: 600 }}>1 300 ₽</span></div>
      ...
      <div style={T.card}>
        <div style={T.h2}>Понравилось?</div>
        <div style={{ fontSize: 14, color: '#8e8e93', margin: '6px 0 12px', lineHeight: 1.4 }}>Регистрация занимает минуту: e-mail + код. Тестовые данные заменятся вашими реальными объектами и договорами.</div>
...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  async function verify() {
    ...
    try {
      ...
      if (!ok) { showToast('Код не подошёл. Проверьте цифры или запросите код заново.'); return }
  ...
  return (
    <div style={{ ...T.page, maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={T.card}>
        {stage === 'landing' ? (
          <>
            <div style={h0}>Roomio — аренда без тетрадок и споров</div>
            <div style={{ ...T.small, margin: '0 0 12px' }}>
              Сервис учёта аренды для арендодателей и арендаторов. Работает в браузере и в Telegram.
            </div>
            <div style={{ fontSize: 14, color: '#1d1d1f', lineHeight: 1.5, margin: '0 0 12px' }}>
            ...
            <button onClick={() => setStage('email')} style={grayBtn}>
              Войти по e-mail
            </button>
            <div style={{ ...T.tiny, margin: '12px 0 0', textAlign: 'center' }}>
              В Telegram — открывается через бота @roomiorent_bot.{' '}
              <button style={{ ...demoBtn, fontSize: 12 }} onClick={() => setPolicyOpen(true)}>Политика конфиденциальности</button>
            </div>
          </>
        ) : stage === 'email' ? (
          <>
            <div style={h0}>Вход в Roomio</div>
            ...
            <button disabled={busy} onClick={sendCode} style={{ ...blueBtn, opacity: busy ? 0.6 : 1 }}>
          ...
          <>
            <div style={h0}>Код из письма</div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button disabled={busy} onClick={verify} style={{ flex: 1, padding: 13, borderRadius: 12, border: 'none', background: '#0071e3', color: '#fff', fontWeight: 700, fontSize: 16, cursor: 'pointer', opacity: busy ? 0.6 : 1 }}>
                {busy ? 'Проверка…' : 'Войти'}
              </button>
              <button onClick={() => setStage('email')} style={{ flex: 1, padding: 13, borderRadius: 12, border: 'none', background: '#e8e8ed', fontWeight: 600, fontSize: 16, cursor: 'pointer' }}>
                Другой e-mail
              </button>
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 10 }}>
              <button disabled={busy} onClick={sendCode} style={demoBtn}>Отправить код ещё раз</button>
            </div>
